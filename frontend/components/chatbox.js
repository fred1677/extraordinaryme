...
export function renderChatbox(targetElement, options = {}) {
    ...
    // ========================================================================
    // 🚀 VOICE CHAT ENGINE (Wired to the index.js Header Toggle)
    // ========================================================================
    ...
    if (SpeechRecognition) {
        ...
        // Keeps the microphone open if voice mode is still active
        recognition.onend = () => {
    ...
    // Listens for the button click from the Header in index.js
    window.addEventListener('tao-voice-toggled', (e) => {
        ...
        if (isVoiceMode) {
            ...
            window.speechSynthesis.cancel(); // Instantly stops AI speaking if toggled off
        }
    });
    ...
    const speakResponse = (text) => {
        ...
        // Half-Duplex: Stop listening so the AI doesn't hear itself speak
        if (recognition) recognition.stop(); 

        const utterance = new SpeechSynthesisUtterance(cleanText);
        utterance.rate = 1.05; 
        
        // Resume listening the moment the AI finishes talking
        utterance.onend = () => {
    ...
    const handleSend = async () => {
        ...
        if (['logout', 'log off', 'sign out', 'sign off', 'shut down', 'shutdown', 'shut-down'].includes(lowerCmd)) {
            ...
            if (currentMode === 'backend') {
                ...
                if (backendWorkspace) {
                    backendWorkspace.innerHTML = ''; 
                    backendWorkspace.style.display = 'none';
                }
                setTimeout(async () => {
                    try {
        ...
        if (['enter backend', 'tao mode'].includes(lowerCmd)) {
            ...
            if (activeUserId) {
                try {
                    ...
                    console.error("[Security] Live check failed:", err);
                }
            }
            ...
            try {
                ...
                appendMessage('system', 'Failed to initialize security protocol.');
            }
            return;
        }
        ...
        if (['exit backend', 'user mode'].includes(lowerCmd)) {
            ...
            try {
                ...
                console.error('[Chatbox] Failed to restore home screen:', err);
            }
            return;
        }
